...
type SelectSubscriberConfig struct {
	SubscriberConfig
...
func (c *SelectSubscriberConfig) setDefaults() {
...
func (c SelectSubscriberConfig) validate() error {
...
// SelectSubscriber makes SELECT queries on the chosen table with the interval defined in the config.
// The rows are unmarshaled into Watermill messages.
type SelectSubscriber struct {
	consumerIdBytes  []byte
	consumerIdString string

	db     *sql.DB
	config SelectSubscriberConfig
...
func NewSelectSubscriber(db *sql.DB, config SelectSubscriberConfig, logger watermill.LoggerAdapter) (*SelectSubscriber, error) {
	if db == nil {
	...
	if err != nil {
		return &SelectSubscriber{}, errors.Wrap(err, "cannot generate subscriber id")
	}
	logger = logger.With(watermill.LogFields{"subscriber_id": idStr})

	sub := &SelectSubscriber{
...
func (s *SelectSubscriber) Subscribe(ctx context.Context, topic string) (o <-chan *message.Message, err error) {
	if s.closed {
...
func (s *SelectSubscriber) consume(ctx context.Context, topic string, out chan *message.Message) {
...
func (s *SelectSubscriber) sendMessage(
	ctx context.Context,
	msg *message.Message,
	out chan *message.Message,
	logger watermill.LoggerAdapter,
...
func (s *SelectSubscriber) Close() error {
	if s.closed {
		return nil
	}
...
func (s *SelectSubscriber) SubscribeInitialize(topic string) error {

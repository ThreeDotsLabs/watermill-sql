...
type BinlogOffsetAdapter interface {
	...
	ConsumedMessageQuery(topic string, offset int, consumerGroup string, logName string, position uint32) (string, []interface{})
	...
	// PositionQuery returns the SQL query and arguments which should return saved position.
	// It returns log_name, log_position
	PositionQuery(topic, consumerGroup string) (string, []interface{})
